...
export default function ProductEditForm({
   ...
   * 1. If a new file is selected, it uploads the new image to Supabase Storage and gets its public URL.
   * 2. It prepares the final product data, including the potentially updated image URL.
   * 3. It calls the `editProduct` server action to update the product in the database.
   * 4. It handles any errors from the server action and updates the form's error state.
   * 5. On success, it refreshes the current route's data and redirects to the product catalog.
   */
  const onSubmit = async (data: EditProductData) => {
    ...
    try {
      // If a new image file is selected for upload, handle the upload process.
      if (selectedFile) {
        ...
        // Upload the new image to the 'product-images' bucket in Supabase.
      ...
      // Consolidate the final product data for the update operation.
      const finalProductData = {
        ...data,
        image_url: finalImageUrl || '',
      };

      // Call the server action to update the product details.
      ...
      // Refresh server-side props and then redirect to show the success banner.
      router.refresh();
      router.push('/merchant/product-catalog?product-edited=true');
    } catch (error) {
      setError('root', {
  ...
  return (
    <>
      <PageTitle>Edit {product.name} Details</PageTitle>
            ...
            >
              ...
              >
                {isSubmitting || isUploading ? (
                  <>
                    {isUploading ? 'Uploading Image...' : 'Editting Product...'}
                    <svg
